...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container and place.ant.can_contain(self)==True:
                place.ant.store_ant(self)
            elif self.is_container and self.can_contain(place.ant)==True:
    ...
    def double(self):
        ...
        if self.is_doubled!=True:
            ...
            if self.place.ant.is_container and self.place.ant.ant_contained!=None and self.place.ant.ant_contained.is_doubled!=True:
                self.place.ant.ant_contained.damage*=2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        pop=self.place
        step=0
        while pop.entrance!=None:
            if not pop.is_hive:
                if len(pop.bees)>0 and step>=self.lower_bound and step<=self.upper_bound:
                    return random_bee(pop.bees)
                pop=pop.entrance
                step+=1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        pop=self.place
        target=random_bee(pop.bees)
        if target!=None and self.turns_to_chew==0:
            self.turns_to_chew=self.chewing_turns
            target.reduce_health(target.health)
        elif target!=None and self.turns_to_chew!=0:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while pop:
            if pop.ant!=None:
                pop.ant.double()
                if pop.ant.is_container and pop.ant.ant_contained!=None:
                    pop.ant.ant_contained.double()
            pop = pop.exit
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        pos=self.place
        insect={}
        while pos.is_hive!=True:
            if pos.ant!=None and pos.ant!=self:
                insect[pos.ant]=count
                if pos.ant.is_container and pos.ant.ant_contained!=None:
                    if count!=0:
                        insect[pos.ant.ant_contained]=count
            if pos.bees!=None:
                for i in pos.bees:
                    insect[i]=count
            count+=1
